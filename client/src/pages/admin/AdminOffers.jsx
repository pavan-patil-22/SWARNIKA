...
export default function AdminOffers() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.title.trim() || !formData.discountPercent) {
      toast.error('Please fill in Offer Title and Discount Percentage');
      return;
    }

    await saveOffer(formData, editId);
    toast.success(`Dynamic offer ${editId ? 'updated' : 'created'}!`);
    setShowModal(false);
  };
  ...
  return (
    <div className="space-y-6 text-slate-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {offers.map(offer => (
          <div key={offer.id} className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4 shadow-sm text-slate-800 flex flex-col justify-between">
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-gray-100">
              <button
                ...
                className="p-1.5 bg-amber-50 text-amber-900 rounded hover:bg-gold hover:text-slate-900 text-xs font-bold flex items-center gap-1 border border-gold/30"
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border border-gray-300 text-slate-900 p-2.5 rounded-lg font-mono focus:border-gold"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button type="submit" className="flex-1 bg-gold-gradient text-slate-900 font-bold p-3 rounded-lg shadow-gold-glow">Save Offer</button>
