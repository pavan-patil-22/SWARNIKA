...
export default function AdminBanners() {
  ...
  const openEditModal = (ban) => {
    ...
    setFormData({
      ...
      buttonText: ban.buttonText || 'EXPLORE COLLECTION',
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title.trim()) {
      toast.error('Please fill in Banner Title');
      return;
    }

    await saveBanner(formData, editId);
    toast.success(`Hero Banner ${editId ? 'updated' : 'added'}!`);
    setShowModal(false);
  };
        ...
        {banners.map((banner, index) => (
          <div key={banner.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm flex flex-col md:flex-row items-center justify-between p-4 gap-4 text-slate-800">
            <div className="flex items-center gap-4 w-full md:w-auto">
              ...
              <img src={banner.imageUrl || "https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?auto=format&fit=crop&w=800&q=80"} alt="" className="w-32 h-20 object-cover rounded-xl border border-gold/30" />
              <div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 bg-amber-50 text-amber-900 rounded-lg hover:bg-gold hover:text-slate-900 text-xs font-bold flex items-center gap-1 border border-gold/30"
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. New Festive Launch"
                  className="w-full bg-gray-50 border border-gray-300 text-slate-900 p-2.5 rounded-lg focus:border-gold"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button type="submit" className="flex-1 bg-gold-gradient text-slate-900 font-bold p-3 rounded-lg shadow-gold-glow">Save Banner</button>
