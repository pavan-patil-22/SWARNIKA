...
// Sender address - update this once you verify your own domain on Resend
const FROM_ADDRESS = process.env.EMAIL_FROM || 'SWARNIKA LUXURY HERITAGE <onboarding@resend.dev>';

export const sendEmail = async ({ to, subject, html }) => {
  try {
    // Optional: allow admin to disable email sending via DB setting
    let dbSetting = null;
    try {
      dbSetting = await Setting.findOne();
    } catch (e) {
      console.log("DB setting lookup skipped");
    }

    if (dbSetting?.emailDisabled) {
      console.log(`[Email Simulated Dispatch] To: ${to} | Subject: ${subject}`);
      return true;
    }

    const resend = getResendClient();
    if (!resend) {
      console.log(`[Email Simulated Dispatch - No API Key] To: ${to} | Subject: ${subject}`);
      return true;
    }
    ...
    console.log(`Email dispatched successfully to ${to}. MessageId: ${data.id}`);
    return true;
  } catch (error) {
    console.error("Resend dispatch error (unexpected):", error);
    return false;
  }
};
